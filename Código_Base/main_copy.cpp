...
public:
    DashboardApp(QWidget* parent = nullptr) : QWidget(parent) {
        ...
        QStringList asignaturasDisponibles = getAsignaturasDisponibles();

        for (int i = 0; i < bloqueLabels.size(); ++i) {
            for (int j = 0; j < salaLabels.size(); ++j) {
                QComboBox* asignaturaComboBox = new QComboBox(this);
                asignaturaComboBox->addItems(asignaturasDisponibles);
                gridLayout->addWidget(asignaturaComboBox, i + 1, j + 1);
            }
        }
